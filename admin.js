...
function sourceTypeLabel(type){
  const labels = {
    iptv:"📡 IPTV / M3U8",
    m3u:"📋 M3U Playlist",
    direct_video:"🎞️ فيديو مباشر MP4",
    radio:"📻 Radio / MP3 / AAC",
    rtmp:"🔴 RTMP",
    rtsp:"🟣 RTSP",
    youtube:"▶️ YouTube",
...
async function buildSourceRequestFromDoc(source){
  let items = [];
  if(source.type === "videos"){
    if(!source.libraryId) throw new Error("لم يتم تحديد قائمة تشغيل للفيديوهات");
    ...
    if(!items.length) throw new Error("قائمة التشغيل فارغة");
  }
  return {
...
window.setHichrawiFallback = async(id)=>{
  try{
    ...
    const request = await buildSourceRequestFromDoc(source);
    const user = auth.currentUser;
    if(!user) throw new Error("سجل الدخول من جديد");
    ...
    await setDoc(doc(db,"settings","stream"),{
      fallbackSourceId:id,
      fallbackSourceName:source.name||"",
      fallbackSourceType:source.type||"iptv",
      fallbackUpdatedAt:new Date()
    },{merge:true});

    alert("🛡️ تم تعيين «"+(source.name||"المصدر") +"» كمصدر احتياطي.");
    await loadBroadcastSources();
  }catch(error){
    console.error("setHichrawiFallback:",error);
    alert("❌ تعذر تعيين المصدر الاحتياطي.\n"+(error.message||""));
  }
};
...
window.clearHichrawiFallback = async()=>{
  try{
    ...
    alert("🛡️ تم إلغاء المصدر الاحتياطي.");
    await loadBroadcastSources();
  }catch(error){
    alert("❌ تعذر إلغاء المصدر الاحتياطي.\n"+(error.message||""));
  }
};
...
async function loadBroadcastSources(){
  ...
  try{
    ...
    const fallbackId = streamData.fallbackSourceId || "";
    ...
    sourcesSnap.forEach(item=>{
      ...
      const isFallback = item.id === fallbackId;
        ...
        <div class="source-item ${isActive ? "active" : ""}" data-source-id="${item.id}">
          <div class="source-item-head">
            ...
            <span class="source-badge ${isActive ? "active" : ""}">
              ${isActive ? "🟢 يعمل" : (isFallback ? "🛡️ احتياطي" : "متوقف")}
            </span>
          </div>
          ...
          <div class="source-actions">
            ...
            <button class="btn" onclick="setHichrawiFallback('${item.id}')">
              🛡️ احتياطي
            </button>
    ...
    if(fallbackId){
      const fb = await getDoc(doc(db,"broadcastSources",fallbackId));
      if(fb.exists()){
        const notice=document.createElement("div");
        notice.className="source-help";
        notice.style.cssText="margin:10px 0;padding:10px;border:1px solid #d99f00;border-radius:8px;color:#ffd45a";
        notice.innerHTML="🛡️ المصدر الاحتياطي الحالي: <b>"+String(fb.data().name||"").replace(/</g,"&lt;")+"</b> — <button class="btn" onclick="clearHichrawiFallback()">إلغاء الاحتياطي</button>";
        list.prepend(notice);
      }
    }
...
window.startHichrawiSource = async(id)=>{
  try{
    ...
    if(!snap.exists()){ alert("❌ المصدر غير موجود"); return; }

    const source = snap.data();
    const request = await buildSourceRequestFromDoc(source);
    const user = auth.currentUser;
    if(!user){ alert("❌ انتهت جلسة الإدارة. سجل الدخول من جديد."); return; }

    const idToken = await user.getIdToken();
    showSourceSwitchStatus("pending","🟡 تم إرسال المصدر الجديد... جاري تحضيره قبل تبديل البث.");
