...
// ================= BROADCAST SCHEDULE UI =================
const HICHRAWI_SCHEDULE_KEY = "hichrawi_tv_broadcast_schedule_v1";
...
function loadScheduleSources(){
  ...
  document.querySelectorAll("[data-source-id]").forEach(card=>{
    const id=card.getAttribute("data-source-id"), name=card.getAttribute("data-source-name")||id, type=card.getAttribute("data-source-type")||"";
    if(id){const o=document.createElement("option");o.value=id;o.textContent=name+(type?" — "+type:"");select.appendChild(o)}
  });
  if(previous)select.value=previous;
}
function renderBroadcastSchedule(){
  const body=document.getElementById("scheduleTableBody"); if(!body)return;
  const items=getLocalSchedule().sort((a,b)=>a.time.localeCompare(b.time));
  if(!items.length){body.innerHTML='<tr><td colspan="5" class="schedule-empty">لا توجد برامج في الجدول بعد.</td></tr>';return}
  body.innerHTML="";
  items.forEach((item,index)=>{
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${item.time}</td><td>${item.name}</td><td>${item.type}</td><td>${item.enabled?"🟢 مفعّل":"⚪ متوقف"}</td><td><button class="btn delete" type="button" onclick="removeBroadcastSchedule(${index})">حذف</button></td>`;
    body.appendChild(tr);
  });
}
function addBroadcastSchedule(){
  const time=document.getElementById("scheduleTime")?.value, sel=document.getElementById("scheduleSource");
  ...
  const option=sel.options[sel.selectedIndex], items=getLocalSchedule();
  items.push({time,name:option.textContent,type:"source",sourceId:sel.value,enabled:true});
  saveLocalSchedule(items);renderBroadcastSchedule();
  if(msg)msg.textContent="✅ تمت إضافة البرنامج إلى الجدول المحلي.";
}
function removeBroadcastSchedule(index){const items=getLocalSchedule();items.splice(index,1);saveLocalSchedule(items);renderBroadcastSchedule()}
function clearBroadcastSchedule(){if(confirm("هل تريد مسح جدول البث المحلي؟")){localStorage.removeItem(HICHRAWI_SCHEDULE_KEY);renderBroadcastSchedule()}}
window.addBroadcastSchedule=addBroadcastSchedule;window.removeBroadcastSchedule=removeBroadcastSchedule;window.clearBroadcastSchedule=clearBroadcastSchedule;
document.addEventListener("DOMContentLoaded",()=>{renderBroadcastSchedule();loadScheduleSources();setTimeout(loadScheduleSources,1500)});
