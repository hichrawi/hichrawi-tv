      ...
      list.innerHTML += `
        <div class="source-item ${isActive ? "active" : ""}" data-source-id="${item.id}">
          ...
          <div class="source-help">
            ...
            ${d.type === "videos" ? " — قائمة: " + (d.libraryId || "غير محددة") : (safeUrl ? " — " + safeUrl : "")}
          </div>
...
// ================= RAILWAY SERVER VIDEO LIBRARY =================

let railwayVideosCache = [];

async function loadRailwayVideos(){
  const list = document.getElementById("videoLibraryList");
  if(!list) return;

  try{
    const response = await fetch(STREAM_ENGINE_URL + "/api/videos?ts=" + Date.now(), {
      cache:"no-store"
    });
    ...
    railwayVideosCache = Array.isArray(data.videos) ? data.videos : [];

    // Render server files in a separate block without deleting Firebase videos.
    let serverBox = document.getElementById("railwayServerVideos");
    if(!serverBox){
      serverBox = document.createElement("div");
      serverBox.id = "railwayServerVideos";
      serverBox.className = "video-list";
      list.parentNode.insertBefore(serverBox, list);
    }

    if(!railwayVideosCache.length){
      serverBox.innerHTML = '<div class="library-empty">لا توجد فيديوهات في /app/videos</div>';
      return;
    }

    serverBox.innerHTML = `
      <div style="color:#e5b300;font-weight:700;margin:8px 0">
        📁 فيديوهات السيرفر /app/videos (${railwayVideosCache.length})
      </div>
    `;

    railwayVideosCache.forEach(v=>{
      const title = String(v.name || v.filename || "فيديو")
        .replace(/</g,"&lt;").replace(/>/g,"&gt;");
      const path = String(v.path || "").replace(/</g,"&lt;").replace(/>/g,"&gt;");
      serverBox.innerHTML += `
        <div class="video-row">
          <span>📁</span>
          <div>
            <div class="video-row-title">${title}</div>
            <div class="video-row-url">${path}</div>
          </div>
          <button class="btn add" onclick="addServerVideoToLibrary(${JSON.stringify(v.path || "")})">
            ➕ إضافة
          </button>
        </div>`;
    });
    ...
    console.error("loadRailwayVideos:", error);
    const list = document.getElementById("videoLibraryList");
    if(list){
      let box = document.getElementById("railwayServerVideos");
      if(!box){
        box = document.createElement("div");
        box.id = "railwayServerVideos";
        box.className = "library-empty";
        list.parentNode.insertBefore(box, list);
      }
      box.textContent = "⚠️ تعذر قراءة /app/videos من Railway";
    }
  }
}

window.addServerVideoToLibrary = async(serverPath)=>{
  if(!serverPath){
    alert("❌ مسار الفيديو فارغ");
    return;
  }

  try{
    const existing = await getDocs(collection(db,"videos"));
    const already = existing.docs.some(d => d.data().serverPath === serverPath);

    if(already){
      alert("ℹ️ الفيديو موجود بالفعل في المكتبة.");
      return;
    }

    const match = railwayVideosCache.find(v => v.path === serverPath);
    const title = match?.name || serverPath.split("/").pop();
    ...
    await loadVideos();
    await loadPlaylists();
    alert("✅ تمت إضافة فيديو السيرفر للمكتبة.");
  }catch(error){
    console.error("addServerVideoToLibrary:",error);
    alert("❌ تعذر إضافة الفيديو.");
  }
};

window.refreshRailwayVideos = async()=>{
  await loadRailwayVideos();
};
