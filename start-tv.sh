...
load_announcement() {
    ...
    ANNOUNCEMENT_DATA="$(python3 -c 'import json,sys; from pathlib import Path; p=Path(sys.argv[1]); d=json.loads(p.read_text(encoding="utf-8")) if p.exists() else {}; enabled=bool(d.get("enabled",False)); text=str(d.get("text","") or "").strip();speed=0.5; size=int(float(str(d.get("fontSize",20) or 20).strip().lower().replace("px",""))); bg=str(d.get("bgColor",d.get("backgroundColor","#e00000")) or "#e00000").lstrip("#"); fg=str(d.get("textColor",d.get("color","#ffffff")) or "#ffffff").lstrip("#"); speed=max(1,min(speed,10)); size=max(18,min(size,80)); bg=bg if len(bg)==6 else "e00000"; fg=fg if len(fg)==6 else "ffffff"; print(("true" if enabled and text else "false")+"\t"+text.replace("\t"," ")+"\t"+str(speed)+"\t"+str(size)+"\t0x"+bg+"\t0x"+fg)' "$ANNOUNCEMENT_FILE")"
